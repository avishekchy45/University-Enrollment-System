...
@section('head')
<title>PUC-Admin | Update Student</title>
@endsection

@section('main')
<ul class='navbar-nav d-flex flex-row mainopt'>
    <li class='nav-item'>
        <a class='nav-link text-uppercase options mr-1 mt-2 mb-0 px-5' href="{{ URL::to('addstudent')}}"> Add Student </a>
    </li>
    <li class='nav-item'>
        <a class='nav-link text-uppercase options mr-1 mt-2 mb-0 px-5' href="{{ URL::to('studentlist')}}"> Student List </a>
    </li>
</ul>
...
<div class="mainpage">
    ...
    <form enctype="multipart/form-data" method="POST" action="{{ URL::to('updatestudent/'.$data->id)}}" class="animate__animated animate__zoomIn">
        @csrf
        <h5 class='text-center formheader'>Update Student Form</h5><br>
        <div class="form-group row">
            <label for="teaid" class="col-sm-2 col-form-label">ID</label>
            <div class="col-sm-10">
                <input type="text" class="form-control" id="stuid" name="stuid" value="{{ $data->student_id}}" readonly required>
                @if ($errors->has('stuid'))
